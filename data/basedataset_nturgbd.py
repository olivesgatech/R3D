...
class BaseDataset(Dataset):
    ...
    def normalize_depth(self, file_path):
        depth_data = np.load(file_path)
        depth_min = np.min(depth_data)
        depth_max = np.max(depth_data)
        if depth_max > depth_min:
            depth_normalized = (depth_data - depth_min) / (depth_max - depth_min) * 255
        else:
            depth_normalized = depth_data#np.zeros_like(depth_data, dtype=np.float32)  # Handle uniform depth case
    ...
    def _make_input(self, vid_file, obs_perc ):
        ...
        depth_features = np.load(depth_feature_file)#self.normalize_depth(depth_feature_file)
