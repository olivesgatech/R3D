...
class BaseDataset(Dataset):
    ...
    def get_gaze_file(self, vid_file):
        # Split the vid_file path to get the activity name and video ID
        base_dir = "/home/seulgi/work/gaze"
        parts = vid_file.split("/")
        
        # Extract the activity name (e.g., "Playing video game")
        activity_name = parts[-3]

        # Extract the video ID from the filename (e.g., "01_2" from "01_2_00023.jpg")
        video_id = os.path.splitext(parts[-1])[0].split('_')[0] + "_" + os.path.splitext(parts[-1])[0].split('_')[1]

        # Construct the gaze_file path
        gaze_file = os.path.join(base_dir, activity_name, f"{video_id}.csv")
    ...
    def _make_input(self, vid_file, obs_perc, seq_idx):
        ...
        with open(gt_file, 'r') as file_ptr:
            ...
            for line in lines:
                if len(line.strip().split(',')) == 3:
                    gaze_file = self.get_gaze_file(line.split(',')[0])
        ...
        ############################## Added Gaze file ################################################
        gaze_file = self.get_gaze_file(input_image_path[0])
        gaze_data = pd.read_csv(gaze_file)
        
        gaze_x = gaze_data[' gaze_x [px]'].values
        gaze_y = gaze_data[' gaze_y [px]'].values
        gaze_x = (gaze_x - gaze_x.min()) / (gaze_x.max() - gaze_x.min())
        gaze_y = (gaze_y - gaze_y.min()) / (gaze_y.max() - gaze_y.min())

        

        # Stack the FFT-transformed gaze_x and gaze_y
        #query = np.stack([fft_gaze_x, fft_gaze_y], axis=1)


        query = np.stack([gaze_x, gaze_y], axis=1)

        observed_gaze_len = int(obs_perc * len(query))
        query_label = query[:observed_gaze_len]
        ###############################################################################################
        ...
        item = {
            ...
            'query_label': torch.tensor(query_label, dtype=torch.float32),  # L3 labels as query
    ...
    def my_collate(self, batch):
        ...
        for item in batch:
            if item is None:
                ...
                b_features.append(item['features'])
                b_past_label.append(item['past_label'])
                b_trans_future_dur.append(item['trans_future_dur'])
                b_trans_future_target.append(item['trans_future_target'])
                b_trans_query_label.append(item['query_label'])
                b_image_path.append(item['image_path'])
        ...
        b_trans_query_label = torch.nn.utils.rnn.pad_sequence(b_trans_query_label, batch_first=True, padding_value=0)
