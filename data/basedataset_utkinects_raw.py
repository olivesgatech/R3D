...
class BaseDataset(Dataset):
    ...
    def _make_input(self, vid_file, obs_perc ):
        ...
        img_folder = os.path.join(self.features_path, vid_file.split('.')[0])
        ...
        depth_folder = os.path.join(self.depth_features_path, vid_file.split('.')[0])
        depth_files = sorted([f for f in os.listdir(depth_folder) if f.endswith('.xml')],
                             key=lambda x: int(re.search(r'\d+', x).group()))

        depth_features = [self.xmltodepth(os.path.join(depth_folder, f)) for f in depth_files]
