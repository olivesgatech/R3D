...
# RGB AVI 파일 목록 가져오기 (S005로 시작하는 파일만 필터링)
avi_files = glob(os.path.join(rgb_video_dir, "*.avi"))
avi_files = [f for f in avi_files if os.path.basename(f).startswith("S003")]

for avi_path in avi_files:
    video_name = os.path.basename(avi_path)  # e.g., S005C001P001R001A001_rgb.avi
    base_name = video_name.replace("_rgb.avi", "")  # S005C001P001R001A001
