...
def validate(model, val_loader, criterion, pad_idx, device):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            outputs = model((features, past_label), depth_features, mode='val')
            ...
            loss_class, n_class_correct, n_class_total, _ = cal_performance(output.view(-1, output.size(-1)),
                                                                            trans_future_target.view(-1), pad_idx, exclude_class_idx=47, reference=get_last_non_padding_labels(past_label, pad_idx), target_ref=trans_future_target[:,0])
            #loss_class, n_class_correct, n_class_total, _ = cal_performance(output.view(-1, output.size(-1)), trans_future_target.view(-1), pad_idx)#, exclude_class_idx=120)
            losses += loss_class
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.seg :
                ...
                #loss_seg, n_seg_correct, n_seg_total, _ = cal_performance(output_seg, target_past_label, pad_idx, exclude_class_idx=None)
                
                loss_seg, n_seg_correct, n_seg_total, _ = cal_performance(output_seg, target_past_label, pad_idx, exclude_class_idx=47, reference=None, target_ref=None)
                losses += loss_seg
            ...
            if args.anticipate :
                ...
                out = out.view(B, -1)
                #loss, n_correct, n_total, _ = cal_performance(output, target, pad_idx, exclude_class_idx=None)
                loss, n_correct, n_total, _ = cal_performance(output, target, pad_idx, exclude_class_idx=47, reference=first_target_past_label, target_ref=target_first)
