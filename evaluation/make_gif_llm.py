...
from model.chatgpt4o_api import get_fine_grained_labels
from utils import generate_prompt, cal_acc_l3
...
def weighted_accuracy_without_gif(log, pred, gold, t_n_labels, actions_dict, exclude_class_idx=None, image_base=None, label_base=None, image_target=None, gif_name=None, duration=0.2, weight_same=1.0, weight_different=10.0):
    ...
    for i in range(length):
        ...
        if pred[i].item() == gt:
            total_weighted_correct += 1#weight

        total_weighted_labels += 1#weight

        log.write(f"\t{gold[i].replace(' ', '')}\t{pred[i].item()}\t{weight}\n")

    weighted_accuracy = total_weighted_correct / total_weighted_labels if total_weighted_labels > 0 else 0
    print("out of: ", total_weighted_labels, ", correct: ", total_weighted_correct)
    return weighted_accuracy
...
def predict(model, vid_list, args, obs_p, n_class, actions_dict, device, query_dict):
    ...
    with torch.no_grad():
        ...
        for vid in vid_list:
            ...
            while True:
                with open("/home/seulgi/work/darai-anticipation/FUTR_proposed/save_dir/darai/visualization/file_baseline_203050/gt_pred_log_{}_{}.txt".format(log_idx, obs_p), "w") as log:
                    ...
                    #future_content = all_content[past_len_90 : past_len_90 + 8 * 15]
                    ...
                    ################## fine-grained label generator ##########################
                    llm_input_image = transform(image_base)
                    time = len(label_base)
                    label_base_2_cls = [actions_dict[label.replace(' ', '')] for label in label_base]
                    
                    human_prompt = generate_prompt(torch.tensor(label_base_2_cls).unsqueeze(0), time)
                    labels_list = []
                    _, labels = get_fine_grained_labels(llm_input_image, human_prompt, time)
                    if len(labels) > time:
                        labels = labels[:time]
                    if len(labels) < time:
                        labels += [47] * (time - len(labels))
                    labels_list.append(labels)
                    labels_list = torch.tensor(labels_list).to(device)
                    ##########################################################################

                    # Model inference
                    outputs = model(inputs=inputs.unsqueeze(0), query=labels_list, mode='test', epoch=log_idx, idx=obs_p)

                    
                    n_l3_correct, n_l3_total = cal_acc_l3(labels_list.view(-1), query_label.view(-1), 47, 48)
                    total_l3 += n_l3_total
                    total_l3_correct += n_l3_correct
                    print("Out of ", total_l3, " , correct: ", total_l3_correct)
        ...
        print('l3 acc : ', total_l3_correct / total_l3)
