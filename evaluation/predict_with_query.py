...
def predict(model, vid_list, args, obs_p, n_class, actions_dict, device, query_dict):
    ...
    with torch.no_grad():
        ...
        for vid in vid_list:
            ...
            query_seq = gt_read.read().split('\n')[:-1]
            gt_read.close()

            L2_label = file_name.split('_')[-1].split('.')[0]
            file_length = len(query_seq)
            gt_seq = [L2_label] * file_length
            ...
            query_seq = np.array(query_seq[:past_len])
            query = query_seq[::sample_rate]
