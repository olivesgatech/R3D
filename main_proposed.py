...
from data.basedataset_proposed_breakfast import BaseDataset
...
#from predict_with_query import predict
from predict_breakfast import predict
...
def main(seed):
    ...
    query_mapping_file = os.path.join(data_path, 'mapping.txt')
    query_dict = read_mapping_dict(query_mapping_file)

    mapping_file = os.path.join(data_path, 'mapping_l2.txt')
    ...
    if args.predict :
        ...
        if args.dataset == 'breakfast' :
            model_path = '/home/seulgi/work/darai-anticipation/FUTR_proposed/save_dir/breakfast/long/model/transformer/1/i3d_transcript/runs0_baseline/checkpoint19.ckpt'#'./ckpt/bf_split'+args.split+'.ckpt'
        ...
        for obs_p in obs_perc :
            for seed in seed_list:
                model_path = f'/home/seulgi/work/darai-anticipation/FUTR_proposed/save_dir/breakfast/long/model/transformer/1/i3d_transcript/runs0/seed_{seed}_best.ckpt'
                model.load_state_dict(torch.load(model_path))
                model.to(device)
                predict(model, video_test_list, args, obs_p, n_class, actions_dict, device, query_dict)

            # print("obs_p: ", obs_p)
            # #testset = BaseDataset(video_test_list, actions_dict, features_path, gt_path, pad_idx, n_class, n_query=args.n_query, args=args, query_dict=query_dict, mode='test', obs_perc=obs_p)
            # #test_loader = DataLoader(testset, batch_size=1, shuffle=False, num_workers=1, collate_fn=testset.my_collate)
            
            # model.load_state_dict(torch.load(model_path))
            # model.to(device)
            # predict(model, video_test_list, args, obs_p, n_class, actions_dict, device, query_dict)
            # #predict(model, test_loader, pad_idx, device)
    else :
        # Training
