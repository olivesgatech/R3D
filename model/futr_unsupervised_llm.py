...
#from model.llava_api_eval import get_fine_grained_labels
...
class FUTR(nn.Module):
    ...
    def __init__(self, n_class, hidden_dim, src_pad_idx, device, args, n_query=8, n_head=8,
        ...
        self.transformer = Transformer(hidden_dim, n_head, num_encoder_layers, num_decoder_layers,
                                        hidden_dim*4, normalize_before=False, query_num=query_num, hidden_dim=hidden_dim, max_seq_len=max_seq_len, device=device)
        ...
        # LLM-based Label Predictor
        #self.llm = VideoToLabelLLM(embedding_dim=hidden_dim)
    ...
    def forward(self, inputs, query, mode='train', epoch=0, idx=0, human_prompt='', image_path=None):
        if mode == 'train' :
        ...
        ######## ADDING #########
        #action_query, attention_map = self.l3_attention(src, src, src) # Action query: shape: (4, t, 128)
        
        
        #########################
        src = rearrange(src, 'b t c -> t b c') # (4, t, 128)
        tgt = torch.zeros_like(src)
        
        pos = rearrange(pos, 'b t c -> t b c')

        src, tgt, l3_logits = self.transformer(src, tgt, src_key_padding_mask, tgt_mask, None, labels_list=query, pos_embed=pos, tgt_pos_embed=None, epoch=epoch, idx=idx, human_prompt=human_prompt, image_path=image_path)
        #src, tgt, l3_logits = self.transformer(src, tgt, src_key_padding_mask, tgt_mask, None, pos_embed=pos, tgt_pos_embed=None, epoch=epoch, idx=idx, human_prompt=human_prompt, image_path=image_path)
