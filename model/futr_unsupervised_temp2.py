...
class FUTR(nn.Module):
    ...
    def forward(self, inputs, query, mode='train', epoch=0, idx=0):
        ...
        src_l3 = pos_embed_l3.to(self.device) + src_l3
        l3_logits = rearrange(src_l3, 'b t c -> t b c')
        #########################
        ...
        src_l3 = src + l3_logits

        src_l3, tgt = self.transformer(src=src_l3, tgt=tgt, mask=src_key_padding_mask, tgt_mask=tgt_mask, tgt_key_padding_mask=None, query_embed=action_query, pos_embed=pos, tgt_pos_embed=None, epoch=epoch, idx=idx)
        ...
        l3_logits = rearrange(l3_logits, 't b c -> b t c')
        if self.args.seg :
            # action segmentation
