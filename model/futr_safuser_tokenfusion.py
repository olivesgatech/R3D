...
class CMFuser(nn.Module):
    ...
    def token_fusion(self, rgb_feats, depth_feats, mode):
        ...
        if mode == 'train':
            loss = (rgb_feats.mean() + depth_feats.mean())
            grads = torch.autograd.grad(loss, [rgb_feats, depth_feats], retain_graph=True)

            grad_rgb = grads[0].abs().mean(dim=(0, 1), keepdim=True)  # (1, 1, C)
            grad_depth = grads[1].abs().mean(dim=(0, 1), keepdim=True)  # (1, 1, C)

        else:
            # Activation Magnitude based
            grad_rgb = rgb_feats.abs().mean(dim=(0, 1), keepdim=True)  # (1, 1, C)
            grad_depth = depth_feats.abs().mean(dim=(0, 1), keepdim=True)  # (1, 1, C)

        k = C // 4
        _, topk_rgb = torch.topk(grad_rgb, k, dim=-1, largest=False)
        _, topk_depth = torch.topk(grad_depth, k, dim=-1, largest=False)

        exchanged_rgb = rgb_feats.clone()
        exchanged_depth = depth_feats.clone()

        exchanged_rgb[:, :, topk_rgb.squeeze()] = depth_feats[:, :, topk_rgb.squeeze()]
        exchanged_depth[:, :, topk_depth.squeeze()] = rgb_feats[:, :, topk_depth.squeeze()]

        stacked_feats = torch.stack([exchanged_rgb, exchanged_depth], dim=2)  # (B, T, 2, C)
        #stacked_feats = stacked_feats.permute(0, 2, 1, 3) # (B, M, T, C)
        #fused_feats = self.fusion_conv(stacked_feats).squeeze(1)  # (B, C, T)
