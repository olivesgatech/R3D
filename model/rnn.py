...
class FUTR(nn.Module):
    ...
    def __init__(self, n_class, hidden_dim, src_pad_idx, device, args, n_query=8, n_head=8,
        ...
        super().__init__()
        self.rnn = nn.LSTM(hidden_dim, hidden_dim // 2, num_layers=2, 
                           batch_first=True, bidirectional=True, dropout=0)
        self.rnn_fc = nn.Linear(hidden_dim, hidden_dim)
    ...
    #def forward(self, inputs, query, mode='train', epoch=0, idx=0):
        ...
        if self.args.input_type == 'i3d_transcript':
            ...
            src = self.gt_emb(src)
        src = F.relu(src)
        rnn_out, _ = self.rnn(src)  # [B, S, hidden_dim]
        
        tgt = self.rnn_fc(rnn_out)  # dimension
